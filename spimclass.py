...
class Spim:
    ...
    def initialization(self):
        ...
        self.x_range = np.linspace(self.xstep_value, self.xstep_value*self.xstep_number, self.xstep_number,
                                   dtype='>f4') # Building 1D array of X positions
    ...
    def initialization_winspec32(self, xstep_number=None, ystep_number=None, xstep_value=None, ystep_value=None,
        ...
        if self.xstep_number is None or self.ystep_number is None:
        ...
        if self.xstep_value is None or self.ystep_value is None:
        ...
        # Create the tab of wavelength.
        self.tab_lambda = column_1[0:self.CCD_nb_pixel]
    
    def define_space_range(self, area=None, coordinate_file=None):
        """Define the size of the area in micrometer of the intensity image and generate the x & y scanned positions.
        
        Parameters
        ----------
        area : tuple of 4 float: ``(xmin, xmax, ymin, ymax)``
               If ``None`` and ``coordinate_file`` is also ``None``, then ``area = (0, 100, 0, 100)`` is supposed.
        ...
        if coordinate_file is None:
            ...
            space_area = area # Define plot area in µm: (xmin, xmax, ymin, ymax).
            pixel_area = []   # Define plot area in pixel.
    ...
    def intensity_map(self, center=None, width=None):
        ...
        image_data = np.sum(
            self.matrix[self.xpmin[0]:self.xpmax[0]+1, self.ypmin[0]:self.ypmax[0]+1,
...
class SpimInterface:
    ...
    def __init__(self, spim, lambda_init=None):
        ...
        self.ax_image.grid(lw=0.2, color='k')
